...
.. note:: When Archon is initialized the environment variable ``IPFS_GATEWAY_URI`` is set. A gateway is needed to access ``IPFS`` files that might be referenced in evidence or metaEvidence.
